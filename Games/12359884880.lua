...
local CollectRemote = ReplicatedStorage:WaitForChild("RemoteCalls"):WaitForChild("GameSpecific"):WaitForChild("Lobby"):WaitForChild("Collecting"):WaitForChild("CollectedSpeed")
...
--Game Features
Utility:AddToggle("AutoCollect", {
	["Text"] = "AutoCollect",
	["Default"] = false,
	["Tooltip"] = "Auto collects speed",
	["Callback"] = function(callback)
		if callback then
			AutoCollectVar = true
			while AutoCollectVar and task.wait() do
				CollectRemote:FireServer(3)
				CollectRemote:FireServer(2)
				CollectRemote:FireServer(1)
			end
		else
			AutoCollectVar = false
		end
	end
})

local MiniFPSBoost = Blatant:AddButton({
	["Text"] = "MiniFPSBoost",
	["Func"] = function()
		workspace.Important.Doors:Destroy()
		workspace.Important.Collecting:Destroy()
	end,
	["DoubleClick"] = false,
	["Tooltip"] = "Removes something in the game thats not useful"
})