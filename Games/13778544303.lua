...
local ServiceRemote = ReplicatedStorage:WaitForChild("common"):WaitForChild("packages"):WaitForChild("_Index"):WaitForChild("sleitnick_knit@1.5.1"):WaitForChild("knit"):WaitForChild("Services")
local ClickRemote = ServiceRemote:WaitForChild("ClickService"):WaitForChild("RE"):WaitForChild("Click")
local SpinRemote = ServiceRemote:WaitForChild("SpinService"):WaitForChild("RF"):WaitForChild("canSpin")
local BestPetRemote = ServiceRemote:WaitForChild("PetService"):WaitForChild("RE"):WaitForChild("getBest")
...
local ClicksFarmVar = false
local AutoFarmV1Var = false
local AutoFarmV2Var = false
local AutoWheelVar = false
local EquipBestVar = false
...
--Game Features
Utility:AddToggle("ClicksFarm", {
	["Text"] = "ClicksFarm",
	["Default"] = false,
	["Tooltip"] = "Farms clicks",
	["Callback"] = function(callback)
		if callback then
			ClicksFarmVar = true
			while ClicksFarmVar and task.wait() do
...
Blatant:AddToggle("AutoFarmV1", {
	["Text"] = "AutoFarmV1",
	["Default"] = false,
	["Tooltip"] = "Original auto farm",
	["Callback"] = function(callback)
		if callback then
			AutoFarmV1Var = true
			while AutoFarmV1Var and task.wait() do
				HumRootPart.Velocity = Vector3.new(0,0,2000)
			end
		else
			AutoFarmV1Var = false
		end
...
Blatant:AddToggle("AutoFarmV2", {
	["Text"] = "AutoFarmV2",
	["Default"] = false,
	["Tooltip"] = "Better auto farm",
	["Callback"] = function(callback)
		if callback then
			AutoFarmV2Var = true
			while AutoFarmV2Var do
				HumRootPart.Position = StartPart
				wait(3)
				HumRootPart.Position = EndPart
			end
		else
			AutoFarmV2Var = false
		end
...
Utility:AddToggle("AutoWheel", {
	["Text"] = "AutoWheel",
	["Default"] = false,
	["Tooltip"] = "Automatically spins the wheel for you",
	["Callback"] = function(callback)
		if callback then
			AutoWheelVar = true
			while AutoWheelVar and task.wait() do
...
Utility:AddToggle("EquipBest", {
	["Text"] = "EquipBest",
	["Default"] = false,
	["Tooltip"] = "Automatically equips the best pets for you",
	["Callback"] = function(callback)
		if callback then
			EquipBestVar = true
			while EquipBestVar and task.wait() do
				BestPetRemote:FireServer()
			end
		else
			EquipBestVar = false
		end
