...
local BadgeRemote = ReplicatedStorage:WaitForChild("Events"):WaitForChild("Player"):WaitForChild("Badge")
...
function teleportTo(location)
	if Char and HumRootPart then
		...
		local teleporting = TweenService:Create(HumRootPart, TweenInfo, { CFrame = cf })
		teleporting:Play()
...
Blatant:AddToggle("AutoFarm", {
	...
	["Callback"] = function(callback)
		if callback then
			FarmVar = true
			while FarmVar do
				for _, location in pairs(teleportposition) do
					teleportTo(location)
					wait(0.5)
				end
			end
...
local FreeBadges = Utility:AddButton({
	["Text"] = "FreeBadges",
	["Func"] = function()
		for _, BadgeID in pairs(BadgesList) do
			BadgeRemote:FireServer(BadgeID)
			notify("You got the badge: " .. BadgeID)
		end
	end,
	["DoubleClick"] = false,
	["Tooltip"] = "Gives you all badges in the game"
})